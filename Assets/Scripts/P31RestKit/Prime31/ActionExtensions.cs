...
namespace Prime31
{
	public static class ActionExtensions
	{
		private static void invoke(Delegate listener, object[] args)
		{
			...
			{
				Debug.LogError(string.Format("an event listener is still subscribed to an event with the method {0} even though it is null. Be sure to balance your event subscriptions.", listener.Method.Name));
			}
			else
			{
